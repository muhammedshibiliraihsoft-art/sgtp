...
Objective: model shop-scoped clients and approved related-person relationships. Dependencies: Phase 3. Files: `backend/apps/clients/{models,migrations,serializers,views,tests}`. Steps: define identity/contact fields, soft deletion, uniqueness, relationship rules, privacy, and the unresolved billing-ownership decision interface. Billing ownership for work belonging to a Related Person must be defined before billing implementation; the approved rule must specify whether the invoice is owned by the primary Client, the Related Person, or another explicitly approved model. Do not infer or implement that billing behavior in T4-01. DB: FKs/indexes/constraints. API: CRUD/search. Security: shop scope/PII. Tests: isolation/validation and relationship ownership cases once approved. DoD: records and the billing-ownership dependency are explicitly documented.
